...
/**
 * Vaadin configuration dialog for IN.
 *
 * @author Vincenzo Cutrona
 */
public class INVaadinDialog extends AbstractDialog<INConfig_V1> {

    private GridLayout propertiesGridLayout;
    private ComboBox langs;

    public INVaadinDialog() {
        super(IN.class);
    }

    @Override
    public void setConfiguration(INConfig_V1 c) throws DPUConfigException {
        ...
        for (int i = 0; i < property.size(); ++i) {
            if (!property.get(i).trim().equals("") && !property.get(i).trim().equals("")) {
                this.addColumnToPropertyMapping(property.get(i), lang.get(i));
            }
        }
        if (property.size() == 0)
    ...
    @Override
    public INConfig_V1 getConfiguration() throws DPUConfigException {
        final INConfig_V1 c = new INConfig_V1();
        ...
        if (!error.isEmpty()) {
            ...
            for (int row = 1; row < this.propertiesGridLayout.getRows(); ++row) {
                String txtProperty = ((TextField) this.propertiesGridLayout.getComponent(0, row)).getValue();
                String txtLang = (String) ((ComboBox) this.propertiesGridLayout.getComponent(1, row)).getValue();
                if (!txtProperty.isEmpty() && !txtLang.isEmpty()) {
    ...
    public void buildDialogLayout() {
        ...
        Button btnAddRow = new Button(ctx.tr("IN.button.add"));
    ...
    private void addColumnToPropertyMapping(final String property, final String lang) {
        ...
        copy.setContainerDataSource(this.langs);
        ...
        final Button btnRemoveRow = new Button(ctx.tr("IN.button.remove"));
    ...
    private void addColumnToPropertyMappingsHeading() {
        this.propertiesGridLayout.addComponent(new Label(ctx.tr("IN.property")));
        this.propertiesGridLayout.addComponent(new Label(ctx.tr("IN.lang.tag")));
        this.propertiesGridLayout.addComponent(new Label(""));
    }

    private void initComboBox() {
        INConfig_V1 c = new INConfig_V1();
    ...
    public String fieldValidation(int column, int row) {
        ...
        if (column == 0) {
            ...
            if (txtValue.isEmpty()) {
                error = error + "\n" + ctx.tr("IN.error.property.not.filled") + " [Row: " + row + "]";
                txt.setComponentError(new UserError(ctx.tr("IN.error.property.not.filled")));
            } else if (!txtValue.startsWith("http://")) {
                error = error + "\n" + ctx.tr("IN.error.property.not.http") + " [Row: " + row + "]";
                txt.setComponentError(new UserError(ctx.tr("IN.error.property.not.http")));
            } else if (txtValue.contains(" ")) {
                error = error + "\n" + ctx.tr("IN.error.property.whitespace") + " [Row: " + row + "]";
                txt.setComponentError(new UserError(ctx.tr("IN.error.property.whitespace")));
            } else {
            ...
            if (txtValue == null || txtValue.toLowerCase().trim().isEmpty()) {
                error = error + "\n" + ctx.tr("IN.error.lang.not.filled") + " [Row: " + row + "]";
                combo.setComponentError(new UserError(ctx.tr("IN.error.lang.not.filled")));
            } else if (txtValue.toLowerCase().trim().contains(" ")) {
                error = error + "\n" + ctx.tr("IN.error.lang.whitespace") + " [Row: " + row + "]";
                combo.setComponentError(new UserError(ctx.tr("IN.error.lang.whitespace")));
            } else {
