...
@DPU.AsQuality
public class ACC6 extends ConfigurableBase<ACC6Config_V1> implements ConfigDialogProvider<ACC6Config_V1> {
    ...
    @Override
    public AbstractConfigDialog<ACC6Config_V1> getConfigurationDialog() {
        return new ACC6VaadinDialog();
    }
        ...
        if ((_subject == null) || (_property == null) || _regExp == null) {
            ...
            for (int i = 0; i < _subject.size(); ++i) {
                ...
                if (!key.trim().isEmpty() && !value.trim().isEmpty() && !regExp.trim().isEmpty()) {
                    ...
                    query1 = "SELECT (COUNT(?s) AS ?counter) WHERE { ?s rdf:type <" + key + "> . }";

                    query2 = "SELECT (COUNT(?s) AS ?counter) WHERE { "
                            + "?s rdf:type <" + key + "> . "
                            + "?s <" + value + "> ?o ."
            ...
            this.createOutputGraph(context, "qualityGraph1", _subject, _property, results);
        }
    }
